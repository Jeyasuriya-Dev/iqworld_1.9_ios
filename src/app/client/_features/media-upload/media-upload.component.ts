...
export class MediaUploadComponent implements OnInit {
  ...
  checkedForm!: UntypedFormGroup;
  checkedMediaForm!: UntypedFormGroup;
  ...
  ngOnInit() {
    ...
    this.clientService.getplaylistByplaylistIdNdclientUsername(this.playListId, this.clientUsername).subscribe((res: any) => {
      ...
      this.selectStateList = this.playlist.state_list;
      this.allSelectedStateList = this.playlist.statelistIds;
      // console.log(this.playlist.statelistIds);
      this.allSelectedDistrictList = this.playlist?.districtlistIds;
      this.allSelectedCityList = this.playlist.citylistIds;
      this.allSelectedLocationList = this.playlist.locationlistIds;
      this.allselectedDeviceList = this.playlist.device_listIds;
      this.selectedOptions = this.playlist.scrollid_list;
      this.allSelectedLandmarkList = this.playlist?.landmarklist ? this.playlist?.landmarklist : [];
      ...
      this.getLandmarkByLocationAndClientname();
  ...
  setDefault(setdefault: any, p: any) {
    ...
    let v: any = this.storageService.getCurrentUser()
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.clientService.setPlaylistAsDefault(this.clientUsername, setdefault.checked, this.playlist.state.id, this.playlist.id, this.playlist.isvertical, v.username).subscribe((res: any) => {
  ...
  onFileChangeFile(event: Event) {
    ...
    if (files && files.length) {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        reader.onload = () => {
          // console.log(this.selectedSize);
          if ((this.playlist?.storage?.client_totalmemory - this.playlist?.storage?.client_usedmemory) >= (files[i].size / 1000) && (files[i].size / 1000) <= this.playlist?.storage?.client_totalmemory) {
            // console.log("a");
  ...
  openSchedulePopUp(template: any) {
    if (this.scheduleFormater() == "Scheduled") {
      this.stopSchedule();
    } else {
  ...
  @ViewChild('selectLandmark') selectLandmark!: MatSelect;
  filteredListLandmark: any = [];
  allSelectedState = false;
  allselectedLandmark = false;
  ...
  toggleAllSelectionForLandmark() {
    if (this.allselectedLandmark) {
      this.selectLandmark.options.forEach((item: MatOption) => item.select());
    } else {
      this.selectLandmark.options.forEach((item: MatOption) => item.deselect());
      this.allselectedDeviceList = [];
    }
  ...
  optionClickLandmark() {
    let newStatus = true;
    this.selectLandmark.options.forEach((item: MatOption) => {
      if (!item.selected) {
        newStatus = false;
      }
    });
    this.allselectedLandmark = newStatus;
    this.getDeviceByLocation();
  }
  demo1TabIndex: any;
  ...
  editPlaylist() {
    ...
    let payload = {
      ...
      landmarklist: this.allSelectedLandmarkList,
      scheduletype: "",
    ...
    if (this.isScheduled && this.playlist?.mediainfo?.id == 3) {
      ...
      Swal.fire({
        ...
        html: "<h4 style='color:orange;'>Schedule in progress. Please try again after completion!</h4>",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
    ...
    } else if (this.playlist.isactive && this.playlist?.mediainfo?.id == 4) {
      Swal.fire({
        title: "<b style='color:orange;'>Warning</b>",
        html: "<h4 style='color:orange;'>Please remove the playlist as the default.</h4>",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
        confirmButtonText: "Unset as default"
      }).then((result) => {
        if (result.isConfirmed) {
          this.setDefault({ checked: false }, this.playlist)
        }
      });
    } else {
      let loader = this.matDialog.open(LoaderComponent, {
  ...
  getLandmarkByLocationAndClientname() {
    let payload = {
      location_list: this.allSelectedLocationList,
      clientname: this.playlist.clientname
    }
    this.clientService.getLandmarkByLocationAndClientname(payload).subscribe(res => {
      // console.log(res);
      this.alllandmarklist = res;
      this.filteredListLandmark = res
    })
  }
  ...
  getDeviceByLocation() {
    ...
    let payload = {
      ...
      location_list: this.allSelectedLocationList,
      clientname: this.playlist.clientname,
      landmark_list: this.allSelectedLandmarkList
    }
    console.log(payload);
    this.clientService.getDeviceByLocationFilter(payload).subscribe(res => {
      // console.log(res);
  ...
  removeAllDevices(e: any) {
    // console.log(e);
    if (e.checked) {
      this.filteredListDevice.forEach((e: any) => {
        // console.log(e);
        if (e.isexist) {
          this.forceList.push(e.id);
          e.isexist = false;
        }
      })
    } else {
  ...
  createSplitScreen(type: any, playlist: any, isProVertical: any) {
    ...
    if (type) {
      ...
      if (v) {
        this.router.navigate(['/client-mobile/lay-out', playlist.id],
          { state: { playlist: playlist, type: isProVertical } })
      } else {
        this.router.navigate(['/client/lay-out', playlist.id],
          { state: { playlist: playlist, type: isProVertical } })
      }
  ...
  getAllowedUploadMediaFiles() {
    this.clientService.getAllowedUploadMediaFiles().subscribe(res => {
      ...
      console.log(this.allowedMediaList);
    ...
    this.clientService.getSplitLayoutsByPlaylistId(playlistId, isProVertical == '9:16' ? true : false).subscribe((res: any) => {
      ...
      const refresh = setTimeout(() => {
        ...
        if (v) {
          ...
          this.router.navigate(["/client/upload-media/", playlistId]).then(() => {
            this.ngOnInit();
          });
        }
  ...
  conformEdit(type: any, playlist: any, isProVertical: any, layout: any) {
    if (type) {
      let v = sessionStorage.getItem('clientid')
      if (v) {
        this.router.navigate(['/client-mobile/lay-out', layout.id],
          { state: { playlist: playlist, type: isProVertical, layout: layout } });
      } else {
        this.router.navigate(['/client/lay-out', layout.id],
          { state: { playlist: playlist, type: isProVertical, layout: layout } });
      }
      // this.matDialog.open(LayoutComponent, {
      //   disableClose: true,
      //   data: { playlist: playlist, layout: layout, type: isProVertical }
      // });
    }
  ...
  updateDurationforMediaFiles() {
    Swal.fire({
      title: 'Enter Duration (HH:MM:SS)',
      input: 'text',
      text: 'This duration does not apply to videos.',
      inputAttributes: {
      ...
      if (result.isConfirmed) {
        console.log('Entered time:', result.value);
        let loader = this.matDialog.open(LoaderComponent, {
          panelClass: 'loader-upload'
        })
        this.clientService.updateMediafileDuartion((this.checkedMediaForm?.get('media') as UntypedFormArray).value, result.value).subscribe((res: any) => {
          Swal.fire({
            title: "Updated!!",
            text: res?.message,
            icon: "success"
  ...
  deleteOrRotateFile(type: any) {
    ...
    Swal.fire({
      ...
      // text: type == 'delete' ? "Do You Want To Delete ? " : 'Do you want to rotate the files?',
      icon: "warning",
      showCancelButton: true,
      cancelButtonColor: "#d33",
      confirmButtonColor: "#3085d6",
  ...
  filterStates(searchText: string): void {
    this.filteredListState = this.stateList.filter((state: any) =>
  ...
  filterDistricts(searchText: string): void {
    this.filteredListDistrict = this.alldistrictlist.filter((state: any) =>
  ...
  filterCities(searchText: string): void {
    this.filteredListCity = this.allcitylist.filter((state: any) =>
  ...
  filterLocations(searchText: string): void {
    this.filteredListLocation = this.alllocationlist.filter((state: any) =>
  ...
  filterLandmark(searchText: string): void {
    this.filteredListLandmark = this.alllandmarklist.filter((state: any) =>
      this.isSelectedLandmark(state) || state.toLowerCase().includes(searchText.toLowerCase())
    );
  }
  isSelectedLandmark(stateId: any): boolean {
    return this.allSelectedLandmarkList.some((selectedId: any) => selectedId === stateId);
  }
  ...
  isValidFileType(file: any): boolean {
    ...
    return this.allowedMediaList.some(
      (type: any) => type.mimetype === mimeType && type.filetype === fileExtension
    );
  }
}
