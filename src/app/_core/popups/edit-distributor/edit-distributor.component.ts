...
export class EditDistributorComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // this.onChooseDistrict(this.customer?.district?.name);
    // this.onChooseCity(this.customer?.city?.cityname);
    ...
    this.form = this.formBuilder.group(
      {
        ...
        distributor: [this.customer.distributor, [Validators.required, Validators.pattern('^[A-Za-z]+$')]],
        username: [
  ...
  onsubmit() {
    ...
    if (this.form.controls["email"].valid) {
      ...
      this.clientService.updateDistibutor(this.form.value).subscribe((res: any) => {
        loader.close();
        Swal.fire({
          position: 'center',
          icon: 'success',
